...
class Rule:
    ...
    def get_rules(self):  # 定义规则
        ...
        def parallel(chord_1, key_1, chord_2, key_2):  # 检查是否有平行八五度以及隐伏八五度
            # 检查平行八度和隐伏八度
            eight_2 = False  # 再检查chord_2中是否有八度
            pos = None
            low_2 = None
            high_2 = None
            for i in range(len(chord_2)):
                if chord_2[i] == key_2:
                    eight_2 = True
                    pos = (i, len(chord_2) + 1)  # 记录下八度声部
                    low_2 = chord_2[i]  # 记录声部
                    high_2 = key_2  # 记录声部
                    break
            if not eight_2:
                for i in range(len(chord_2) - 1):
                    for j in range(i + 1, chord_2):
        ...
        def diagonal(chord_1, key_1, chord_2, key_2):  # 检查声部对斜
            for i in range(len(chord_1)):
                if i > len(chord_2):  # 一般而言现在要求和声长度一致
                    break
                for j in range(chord_2):
                    if i == j:
                        continue
